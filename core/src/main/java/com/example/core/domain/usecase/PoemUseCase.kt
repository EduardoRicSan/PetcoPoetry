...
class PoemUseCase @Inject constructor(
    ...
    suspend fun getPoemsByAuthor(authorName: String) : Flow<NetworkResource<PoemTitle>> {
        return poetryRepository.getPoemsByAuthor(authorName)
    }

    suspend fun getPoemInfo(authorName: String, poemTitle: String) : Flow<NetworkResource<PoemInfo>> {
